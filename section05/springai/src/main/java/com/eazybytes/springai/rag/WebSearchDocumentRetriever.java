...
/**
 * WebSearchDocumentRetriever is a DocumentRetriever implementation that
 * retrieves relevant documents from the web using the "TAVILY" search API. It
 * sends the user's query to the API and maps the search results into Spring AI
 * Document objects, which can then be used for retrieval-augmented generation
 * (RAG) in chat applications.
 *
 * The retriever expects an environment variable "TAVILY_SEARCH_API_KEY" to be
 * set with a valid API key. It also allows configuration of the maximum number
 * of search results to retrieve.
 */
public class WebSearchDocumentRetriever implements DocumentRetriever {
	...
	/**
	 * Constructor for WebSearchDocumentRetriever, which initiates a client to
